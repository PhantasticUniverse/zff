...
function setRegionEnvironments() {
    ...
    // Loop through all 16 regions (4x4 grid) // adjusted for 8x8
    for (let y = 0; y < 8; y++) {
        for (let x = 0; x < 8; x++) {
            // Calculate a unique value for each region based on its position
            const regionValue = (x + 1) * (y + 1) / 64;
            ...
            // const impassable = ((x != 0 && x !=7) && (y === 1 | y ===6)) | ((x != 0 && x != 3 && x != 4 && x != 7) && (y === 3 | y ===4));
            // const impassable = (x === 2 | x === 5) | (y === 2 | y === 5);

            // Basic patterns
            const checkerboardPattern = ((x + y) % 2 === 0);
            const gridPattern = (x === 2 | x === 5) | (y === 2 | y === 5);
            const diagonalPattern = (x === y || x === 7 - y);
            const bordersPattern = (x === 0 || x === 7 || y === 0 || y === 7);
            const crossPattern = (x === 3 || x === 4 || y === 3 || y === 4)

            // does not make a spiral
            const spiralPattern = function(x, y) {
                const dx = x - 3.5;
                const dy = y - 3.5;
                const distance = Math.sqrt(dx * dx + dy * dy);
                const angle = Math.atan2(dy, dx);
                return (distance + angle) % 2 < 1;
            };

            main.setRegionImpassable(x, y, gridPattern);
        }
    }
    updateRegionGrid();
}
...
function setColorsFromPhantasticMap() {
    for (let i = 0; i < 256; i++) {
        const color = cmap_phantastic[i];
